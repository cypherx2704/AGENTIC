// Atlas project config for auth-service migrations.
...
//   20260606_0001__init.sql   — schema, tables, RLS, grants
//   20260606_0002__seed.sql   — well-known tenants, plan_defaults, default policy, service_acl
//
...
variable "dev_url" {
  ...
  default = "docker://postgres/16/dev?search_path=auth"
}

env "local" {
  url     = var.url != "" ? var.url : "postgres://auth_user:localdev@localhost:5432/cypherx_platform?search_path=auth&sslmode=disable"
  dev     = var.dev_url
  src     = "file://schema.sql"
  migration {
    dir = "file://."
