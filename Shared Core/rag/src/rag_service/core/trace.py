...
``X-Agent-ID`` from the inbound request and binds them into structlog's contextvars so
every log line carries the correlation fields. Also exposes the parsed values on
contextvars for the rest of the request lifecycle (usage-record provenance).
...
def parse_traceparent(value: str | None) -> tuple[str, str] | None:
    """Parse a W3C ``traceparent`` header into (trace_id_uuid, span_id_hex) or None."""
    if not value:
        return None
...
def current_traceparent() -> str:
    """Build a W3C traceparent header from the current trace/span context vars.

    Used when this service makes a downstream call (e.g. the worker -> llms embeddings)
    so the trace propagates (Contract 8). Falls back to a fresh trace if unset.
    """
    trace_id = trace_id_var.get() or str(uuid.uuid4())
    span_id = span_id_var.get() or uuid.uuid4().hex[:16]
    trace_hex = uuid.UUID(trace_id).hex
    return f"00-{trace_hex}-{span_id}-01"
