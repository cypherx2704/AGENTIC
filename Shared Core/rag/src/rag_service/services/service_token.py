...
RAG authenticates to the llms-gateway (embeddings) as the ``rag`` service principal. It
mints a short-lived (5-minute) SERVICE JWT from Auth via ``POST /v1/service-tokens`` using
...
class ServiceTokenProvider:
    """Acquires + caches RAG service JWTs (one cache entry per ``on_behalf_of``)."""
    ...
    async def _mint(self, on_behalf_of: str | None) -> tuple[str, float]:
        ...
        try:
            ...
            raise ApiError(ErrorCode.SERVICE_UNAVAILABLE, "Unable to mint RAG service token.") from exc
