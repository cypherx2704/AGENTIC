-- =====================================================================================
-- rag-service — FLATTENED schema snapshot (init + seed). Declarative source-of-truth for
-- atlas schema apply / drift detection. Generated from the versioned migrations; keep in sync.
-- =====================================================================================
...
-- Hybrid (lexical) leg — additive (migration 0003). GENERATED ALWAYS so Postgres maintains it
-- transparently; folds an OPTIONAL metadata->>'context' prefix (RAG_CONTEXTUAL_INGEST) at
-- weight 'B' over the content at weight 'A'. Default search stays 'dense' (this is unused then).
...
-- (`auth.service_acl`) is OWNED and seeded by auth-service (its
-- `20260614_0009__service_acl_seed.sql` provisions the rag caller edges). RAG must NOT write
-- into the `auth` schema — a cross-schema seed coupled rag to auth's column names.
-- =====================================================================================
