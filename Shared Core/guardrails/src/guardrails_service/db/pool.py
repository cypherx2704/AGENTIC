...
The runtime role (``grd_user``) is NOT a superuser and does NOT bypass RLS, so every
...
rows for that tenant (plus NULL-tenant platform rows on mixed-scope tables such as
``policies`` and ``rules``).
"""
