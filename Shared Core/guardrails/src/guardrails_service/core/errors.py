...
contracts/api/error-format.schema.json). The guardrails-specific code is
``GUARDRAIL_VIOLATION`` (422) — note: the ``/v1/check/*`` endpoints return 200 with
``decision='block'`` in the body; the CALLER (xAgent) translates a block decision into a
422 GUARDRAIL_VIOLATION. This code is defined here so any direct enforcement path can use it.
"""
...
class ApiError(Exception):
    ...
    def __init__(
        ...
        *,
        ...
        self.status_code = (
            status_code if status_code is not None else _DEFAULT_STATUS.get(code, 500)
        )
