...
correlation fields. Also stores the trace id / request id on contextvars for the
rest of the request lifecycle (violation-record + usage provenance).

Per Component 4 provenance rules (X-Request-ID hardened — Amendment Log 2026-06):
  * ``request_id`` = inbound ``X-Request-ID`` VALIDATED as a UUID. Absent -> synthesise
    a UUIDv4 + WARN ``request_id_generated_fallback=true``. Non-UUID junk -> synthesise
    a UUIDv4 + WARN ``request_id_replaced=true`` with the junk value kept in the
    ``inbound_request_id`` log field for correlation. Downstream rows (violations,
    usage) are ALWAYS written with the synthesised id — a junk header from an external
    caller can never suppress its own violation/billing rows.
...
# Junk X-Request-ID values are kept in a log field for correlation, truncated so an
# abusive caller cannot stuff megabytes into the log line.
_INBOUND_ID_LOG_MAX = 64
...
class TraceContextMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # ── request_id (validated as a UUID — junk is replaced, never trusted) ──
        ...
        if not inbound_request_id:
            ...
            try:
                # Canonicalise (case/braces) so the stored UUID is normalised.
                request_id = str(uuid.UUID(inbound_request_id.strip()))
            except ValueError:
                request_id = str(uuid.uuid4())
                logger.warning(
                    "request_id_replaced",
                    request_id_replaced=True,
                    request_id_generated_fallback=True,
                    # Keep the junk value (bounded) so callers can still correlate.
                    inbound_request_id=inbound_request_id[:_INBOUND_ID_LOG_MAX],
                )
