"""Run entry: ``python -m guardrails_service`` (cross-platform).
...
def main() -> None:
    ...
    if sys.platform == "win32":
        ...
        config = uvicorn.Config(
            "guardrails_service.main:app", host=host, port=port, log_config=None, loop="none"
        )
        ...
        uvicorn.run("guardrails_service.main:app", host=host, port=port, log_config=None)
