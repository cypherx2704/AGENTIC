"""Contract 12 — service-token acquisition + caching (Memory).

Memory authenticates to the llms-gateway (embeddings) as the ``memory`` service principal.
It mints a short-lived (5-minute) SERVICE JWT from Auth via ``POST /v1/service-tokens`` using
the bootstrap secret, caches it in-process per ``on_behalf_of`` until shortly before expiry,
and sends it alongside ``X-Forwarded-Agent-JWT`` so the gateway resolves the CALLER's tenant
BYOK key for the embedding. Mirrors the RAG service's provider.

Auth-call shape (Contract 12):
  * ``X-Service-Name: memory``
  * ``X-Service-Bootstrap-Secret: <SERVICE_BOOTSTRAP_SECRET>``
  * body ``{ "on_behalf_of": "<agent_id>" }`` — downstream verifies it matches the
    forwarded agent JWT's ``agent_id``.
"""
...
class ServiceTokenProvider:
    """Acquires + caches Memory service JWTs (one cache entry per ``on_behalf_of``)."""
    ...
    def enabled(self) -> bool:
        """True only when a bootstrap secret is configured (else fall back to static/mock)."""
        return bool(self._settings.service_bootstrap_secret)
    ...
    async def _mint(self, on_behalf_of: str | None) -> tuple[str, float]:
        ...
        try:
            ...
            raise ApiError(ErrorCode.SERVICE_UNAVAILABLE, "Unable to mint Memory service token.") from exc
