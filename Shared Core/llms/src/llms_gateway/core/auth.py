...
``auth_platform_audience``, ``exp`` must be valid, and scope ``llm:invoke`` is
...
def get_jwks_client(jwks_url: str) -> PyJWKClient:
    ...
    if client is None:
        # 5-minute key cache (Contract 1); lifespan_seconds matches the cluster TTL.
...
def _kid_of(token: str) -> str | None:
    ...
    (a missing kid simply means there is no poisoned-kid rule to check)."""
    try:
...
def _resolve_principal(
    ...
    forwarded agent JWT (INTERNAL). The async revocation lookup runs in
    ``require_principal`` (Valkey is async + lives on ``app.state``).
    """
...
async def _enforce_revocation(
    ...
    unavailable (missing client, connect error, or timeout) the token is ACCEPTED —
    we log ``revocation_check_skipped=true`` and bump a metric. A short per-lookup
    timeout (config: ``revocation_valkey_timeout_seconds``) keeps a slow Valkey from
    ...
    if valkey is None:
        # No Valkey wired (e.g. a minimal test app) — fail open, same as unavailable.
