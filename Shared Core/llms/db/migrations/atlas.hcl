// Atlas project config for llms-gateway migrations (Phase 3).
...
variable "dev_url" {
  ...
  default = "docker://postgres/16/dev?search_path=llms"
}

env "local" {
  url = var.url != "" ? var.url : "postgres://llms_user:localdev@localhost:5432/cypherx_platform?search_path=llms&sslmode=disable"
  dev = var.dev_url
  src = "file://schema.sql"
  migration {
    dir = "file://."
