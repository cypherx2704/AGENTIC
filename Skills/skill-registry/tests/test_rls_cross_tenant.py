...
tenant_id (or NULL, forging a platform skill). The fix is a SEPARATE write policy with
...
1. **Policy-predicate model** — we model the exact ``WITH CHECK`` predicate from the
   ...
   own-tenant row, for skills, skill_versions, skill_capabilities, and skill_health. The
...
from skill_registry.db import queries
...
_MIGRATION = (
    Path(__file__).resolve().parents[1] / "db" / "migrations" / "20260624_0001__init.sql"
...
TABLES = ["skills", "skill_versions", "skill_capabilities", "skill_health"]
...
def test_every_table_has_a_with_check_write_policy(table: str) -> None:
    ...
    pattern = re.compile(
        rf"CREATE POLICY p_\w+_write ON skills\.{re.escape(table)} FOR ALL.*?WITH CHECK\s*\("
...
def test_capabilities_specifically_has_own_with_check_policy() -> None:
    """The prompt calls out skill_capabilities explicitly — assert its own policy exists."""
    assert "CREATE POLICY p_skill_caps_write ON skills.skill_capabilities FOR ALL" in _MIGRATION
    assert "p_skill_caps_write" in _MIGRATION
...
async def test_registration_always_binds_guc_tenant_never_attacker_tenant() -> None:
    ...
    We register a skill as tenant A. Every INSERT/UPDATE the code issues must use the
    ``NULLIF(current_setting('app.tenant_id', true), '')::uuid`` GUC form for tenant_id
    (or a parent skill_id under that tenant) — there is no code path that interpolates a
    ...
    pool.on("INSERT INTO skills",
            [{"skill_id": "tid", "name": "skill-a", "tenant_id": TENANT_A,
              "status": "active", "latest_version": "1.0.0"}])

    await queries.create_skill_with_version(
        pool, TENANT_A,
        name="skill-a", version="1.0.0",
        manifest={"name": "skill-a"},
        capabilities=[("do_thing", "skill:skill-a:invoke")],
    )
    ...
    tenant_inserts = [
        ...
        if w[0].startswith("INSERT INTO skills.") or w[0].startswith("INSERT INTO skill")
        or w[0].startswith("INSERT INTO ")
    ]
    assert tenant_inserts, "expected INSERTs to be captured"
    for sql, _params in pool.writes:
        if sql.startswith("INSERT INTO skill") and "tenant_id" in sql:
...
async def test_with_check_rejection_surfaces_as_error() -> None:
    ...
    def _veto(sql: str, params) -> None:  # type: ignore[no-untyped-def]
        ...
        if "INSERT INTO skill_capabilities" in sql:
