...
from skill_registry.services.health_runner import poll_one
...
BASE = "http://skill-x:8080"

_MANIFEST = {"schema_version": "1.0.0", "name": "skill-x"}
...
async def test_poll_200_changed_with_etag() -> None:
    ...
    assert client.calls[0]["url"] == "http://skill-x:8080/manifest"
...
async def test_poll_one_transitions_active_degraded_offline_active() -> None:
    ...
    state = await poll_one(pool, client, SETTINGS, skill_id=SKILL_ID, base_url=BASE, current=state)
    ...
    # On the 304-recovery poll the prior etag v1 must be sent and the skill returns active.
    state = await poll_one(pool, client, SETTINGS, skill_id=SKILL_ID, base_url=BASE, current=state)
    assert state.status == STATUS_ACTIVE
    ...
    # Each poll persisted a skill_health UPSERT (writes captured by the fake pool).
    health_writes = [w for w in pool.writes if "skill_health" in w[0]]
...
async def test_poll_one_persists_changed_manifest_only_on_200() -> None:
    ...
    state = HealthState()
    await poll_one(pool, client, SETTINGS, skill_id=SKILL_ID, base_url=BASE, current=state)
    # The 200 wrote both skill_health AND the version manifest refresh.
    assert any("skill_versions" in w[0] and "manifest" in w[0].lower() for w in pool.writes)

    pool.writes.clear()
    await poll_one(
        pool, client, SETTINGS, skill_id=SKILL_ID, base_url=BASE,
        current=HealthState(last_etag='"v1"'),
    )
    # The 304 wrote skill_health but NOT a manifest refresh (manifest unchanged).
    assert not any("skill_versions" in w[0] for w in pool.writes)
