...
Each registered skill exposes a ``GET {base_url}/manifest`` endpoint. The registry:

* polls it EAGERLY at registration (so a freshly-registered skill's health is known
  immediately), and
...
advances the failure counter and (eventually) flips the skill to degraded/offline.
...
@dataclass(frozen=True)
class HealthState:
    """The persisted health state of a skill (mirrors the ``skill_health`` row)."""
