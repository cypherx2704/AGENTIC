...
* :func:`poll_one` polls a single skill and persists the resulting health state. Used
  both for the EAGER poll at registration and inside the sweep.
* :func:`sweep_once` polls every registered skill once (platform-scoped read).
...
async def poll_one(
    ...
    *,
    ...
    """Poll one skill's manifest, fold the outcome through the state machine, persist it.
...
def _current_state(row: dict[str, Any]) -> HealthState:
    """Build the current HealthState from a pollable-skills row (health may be absent)."""
    return HealthState(
...
async def sweep_once(
    ...
    """Poll every registered skill once. Returns the number of skills polled."""
    rows = await queries.list_pollable_skills(pool)
    counts: dict[str, int] = {}
    for row in rows:
