...
The runtime role (``skill_user``) is NOT a superuser and does NOT bypass RLS, so every
...
async def in_platform[T](
    ...
    Used by the manifest-health poller (updates ``skill_health`` for every skill) and
