# skill-registry migrations

Idempotent, ordered SQL migrations for the `skills` schema. Apply with any runner
(Atlas, psql, the platform migrate job). Naming: `YYYYMMDD_NNNN__<slug>.sql`.

| File | Purpose |
|------|---------|
| `20260611_0001__init.sql` | `skills`, `skill_versions`, `skill_capabilities`, `skill_health`; indexes; **corrected split RLS + WITH CHECK** (the marketplace-hole fix) on every table incl. `skill_capabilities`; `skill_user` grants. |
| `20260611_0002__seed.sql` | Platform `skill-web-search` skill + version (Contract-4 manifest) + capability/scope row + health row. |
...
2. `*_write` (`FOR ALL`) — `USING` and `WITH CHECK` both = `tenant_id = current_tenant`:
   ...
   write a row carrying another tenant's `tenant_id` (or `NULL` to forge a platform skill)
   is **rejected**. This is applied to every table **including `skill_capabilities`**.
...
`skill_health` additionally has a `*_poller` policy so the empty-GUC background sweep may
update health for **all** skills (incl. tenant-owned) — but only from the trusted
