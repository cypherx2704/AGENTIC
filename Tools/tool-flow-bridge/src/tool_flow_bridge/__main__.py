"""Run entry: ``python -m tool_flow_bridge`` (cross-platform).
...
def main() -> None:
    ...
    if sys.platform == "win32":
        ...
        config = uvicorn.Config(
            "tool_flow_bridge.main:app", host=host, port=port, log_config=None, loop="none"
        )
        ...
        uvicorn.run("tool_flow_bridge.main:app", host=host, port=port, log_config=None)
