...
A Starlette ``BaseHTTPMiddleware`` that rejects any request whose declared
Content-Length exceeds ``settings.max_request_body_bytes`` with a Contract-2 **413
PAYLOAD_TOO_LARGE** envelope BEFORE the route handler. Emits the SAME Contract-2 error
envelope as the exception handlers directly, because a middleware runs OUTSIDE the
FastAPI exception-handler stack.
"""
