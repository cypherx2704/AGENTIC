...
The DB returns the UNION of platform tools (tenant_id IS NULL) and the caller's own
tools (RLS-scoped). This module folds that list into the discovery view:

* **Tenant priority (shadowing):** when a tenant has a tool with the SAME name as a
  platform tool, the tenant's tool WINS — the platform tool is hidden from that
  tenant's discovery. A tenant cannot see another tenant's tool at all (RLS), so the
  only collision possible is tenant-vs-platform.
* **Invoke URL:** resolved from the tool's manifest (``base_url`` when present, else a
  conventional ``http://<name>:8080``) — this is the URL xAgent's tool loop posts to.
...
# Default in-cluster invoke port for a tool server when the manifest omits base_url.
_DEFAULT_TOOL_PORT = 8080


def shadow_by_tenant_priority(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Collapse (name -> tool) keeping the tenant row over a platform row of the same name.

    ``rows`` are tool rows each carrying ``name`` and ``is_platform`` (bool). For each
...
def resolve_invoke_url(manifest: dict[str, Any] | None, name: str) -> str:
    """Resolve the tool's invoke base URL from its manifest, else a conventional name."""
    if isinstance(manifest, dict):
        base = manifest.get("base_url")
        if isinstance(base, str) and base.strip():
            return base.rstrip("/")
    return f"http://{name}:{_DEFAULT_TOOL_PORT}"
...
def build_tool_view(
    ...
    *,
    ...
    """Assemble the discovery response object for one resolved tool.

    Returns the tool's identity, the resolved manifest + version, its invoke URL, the
