"""Run entry: ``python -m tool_registry`` (cross-platform).
...
def main() -> None:
    ...
    if sys.platform == "win32":
        ...
        config = uvicorn.Config(
            "tool_registry.main:app", host=host, port=port, log_config=None, loop="none"
        )
        ...
        uvicorn.run("tool_registry.main:app", host=host, port=port, log_config=None)
