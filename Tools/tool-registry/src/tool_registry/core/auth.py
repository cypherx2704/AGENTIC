...
discovery (GET /v1/tools) only needs an authenticated principal, while registration
(POST) needs ``tool:admin`` or ``platform:admin``. So ``require_principal`` does NOT
...
# Scopes accepted for tool/version registration (POST). A wildcard
# ``tool:*:invoke`` is platform/admin per Contract 4; ``platform:admin`` is the
...
def require_scopes(scopes: tuple[str, ...]) -> Callable[..., Awaitable[Principal]]:
    ...
    Used by registration endpoints (``tool:admin`` / ``platform:admin``).
