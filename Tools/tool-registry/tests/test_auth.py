...
Cases:
  ...
  (c) expired                                          -> 401
  (d) admin endpoint with tool:admin scope             -> 200
...
from tool_registry.core import auth as auth_module
from tool_registry.core.auth import ADMIN_SCOPES, Principal, require_principal, require_scopes
from tool_registry.core.config import Settings
from tool_registry.core.errors import install_exception_handlers
...
def _agent_claims(**overrides: Any) -> dict[str, Any]:
    ...
    claims: dict[str, Any] = {
        ...
        "tenant_id": TENANT, "agent_id": AGENT, "scopes": ["tool:invoke"],
    }
...
def _svc_claims(**overrides: Any) -> dict[str, Any]:
    ...
    claims: dict[str, Any] = {
        ...
        "on_behalf_of": AGENT, "scopes": ["tool:invoke"],
    }
    claims.update(overrides)
    return claims
...
def test_admin_endpoint_with_admin_scope_200() -> None:
    client = _make_client()
    resp = client.post("/admin", headers=_auth(_mint(_agent_claims(scopes=["tool:admin"]))))
    assert resp.status_code == 200, resp.text


def test_admin_endpoint_without_admin_scope_403() -> None:
    client = _make_client()
    resp = client.post("/admin", headers=_auth(_mint(_agent_claims(scopes=["tool:invoke"]))))
