...
- **Keyless local defaults** (`.env.example`): `MOCK_PROVIDERS=true`, `MOCK_EMBEDDINGS=true` / `EMBEDDINGS_FALLBACK_TO_MOCK=true` / `EMBEDDINGS_MOCK_FALLBACK=true` (memory), `CLASSIFIER_MODE=stub`. Flip a toggle off + add the key to go live. (The `web_search` flow-tool is keyless by default too — the Node-RED flow's `SEARCH_PROVIDER` is unset ⇒ mock; set it + a provider key on the platform runtime to go live.)
