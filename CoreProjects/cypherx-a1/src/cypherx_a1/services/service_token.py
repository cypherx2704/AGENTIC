...
cypherx-a1 authenticates to downstream SharedCore services (llms, guardrails, rag,
memory) as the ``cypherx-a1`` service principal. It mints a short-lived (~5-minute)
SERVICE JWT from the Auth service via ``POST /v1/service-tokens`` using the bootstrap
secret, and caches it in-process until shortly before expiry.
...
Auth-call shape (Contract 12):
  ...
  * body ``{ "on_behalf_of": "<agent_id>" }`` — the agent this call serves; downstream
    verifies it matches the forwarded agent JWT's ``agent_id``. The cache is keyed by
    ``on_behalf_of``.

The returned token is used as ``Authorization: Bearer <service-jwt>`` on the downstream
call; the inbound agent JWT is forwarded separately as ``X-Forwarded-Agent-JWT`` by the
downstream clients. Verbatim port of xAgent ax-1's service-token provider.
"""
...
class ServiceTokenProvider:
    """Acquires + caches cypherx-a1 service JWTs (one cache entry per ``on_behalf_of``)."""
    ...
    async def _mint(self, on_behalf_of: str | None) -> tuple[str, float]:
        ...
        try:
            ...
            raise ApiError(ErrorCode.SERVICE_UNAVAILABLE, "Unable to mint cypherx-a1 service token.") from exc
