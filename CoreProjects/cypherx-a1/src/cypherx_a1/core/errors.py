...
Defines :class:`ApiError` plus the FastAPI exception handlers that render every error as::

    { "error": { "code", "message", "details?", "request_id", "trace_id", "timestamp" } }

Error-code constants use the Contract 2 spelling (x-known-codes). cypherx-a1 adds two
product domain codes alongside the canonical set:
  * ``GUARDRAIL_VIOLATION`` -> 422 (copilot input/output blocked by guardrails)
  * ``RESERVED_METADATA_KEY`` is surfaced as ``VALIDATION_ERROR`` with
    ``details.reason="RESERVED_METADATA_KEY"`` (Contract 13 anti-spoof guard).
Mirrors the xAgent ax-1 error module.
"""
...
def install_exception_handlers(app: FastAPI) -> None:
    ...
    async def _handle_validation(_request: Request, exc: RequestValidationError) -> JSONResponse:
        # jsonable_encoder coerces non-JSON-serializable values (e.g. a bytes `input` when the
        # body is not valid JSON) so a malformed body yields a clean 422 — never a 500.
        return _render(
