...
**Status: implemented.** First-cycle (9A) + WP08 reliability (cancel/timeout/idempotency/authorize/sweeper/live-revocation, async mode + SSE) are fully built and tested (27 test files). The WP12 **Tool-loop** stage is **coded, bound, and ENABLED by default** (self-skips for toolless agents) so agents with `allowed_tools` run the LLM↔MCP loop; the remaining WP12 stages (RAG / Memory / Skills) stay **DISABLED by default** (`STAGE_ENABLE_*` flags) pending their phases. Branch: `development`.
...
| `src/agent_runtime/core/stages/` | Concrete stages: `load`, `pre_guardrail`, `prompt_build`, `llm`, `post_guardrail`, `event` (+ WP12 `rag_query`, `memory_retrieve`, `memory_write`, `tool_loop`, `skill_load`); `deps.py` holds the shared clients/Valkey holder bound from the lifespan. `tool_loop` ranks+caps offered tools and sends NO `tool_mode`, so the gateway derives native-vs-emulated per model (a SMALL/8B model is emulated, a frontier model is native); it drops tool calls naming un-offered tools before they reach the history, and falls back to emulated once on a provider `tool_use_failed`; `skill_load` resolves `allowed_skills` + access via `services/skill_registry_client.py` (Skill Registry mirror) and stashes them on `ctx.skills` for PROMPT_BUILD. |
...
- `LLMS_GATEWAY_URL`, `GUARDRAILS_SERVICE_URL` — first-cycle downstreams. `RAG_SERVICE_URL`, `MEMORY_SERVICE_URL`, `TOOL_REGISTRY_URL`, `SKILL_REGISTRY_URL` — WP12/Phase-8 enhancement deps (wired even though stages default-off). Tool-loop knobs: `TOOL_LOOP_MAX_OFFERED_TOOLS` (8), `TOOL_LOOP_TOOL_USE_NUDGE` (true). **`TOOL_LOOP_TOOL_MODE` (auto|native|emulated) must stay UNSET** — leave the mode to the gateway, which derives native-vs-emulated *per model* from `llms.model_capabilities.native_tool_use`. Setting it pins ONE mode across every model and short-circuits that lookup (it forced native tool-calling on `llama-3.1-8b-instant`, which cannot do it → Groq `400 tool_use_failed` → dead task). To change how one model is driven, set its `native_tool_use` row.
...
- **TOOL_LOOP is ON by default (Phase 7); the other WP12 enhancement stages are still OFF.** `tool_loop` is bound AND `enabled=True` by default (`config.py` `stage_enable_tool_loop=True` / `STAGE_REGISTRY`); `rag_query`, `memory_retrieve`, `memory_write` are bound but `enabled=False`; `skill_load` is registry-listed but unbound. Every enhancement stage still SKIPS unless the agent config opts in (`allowed_tools` / `allowed_kb_ids` / memory_scope) — so a **toolless** agent's served pipeline is exactly LOAD→PRE_GUARDRAIL→PROMPT_BUILD→LLM→POST_GUARDRAIL→EVENT (TOOL_LOOP self-skips before any client call), and PROMPT_BUILD is byte-identical to the first cycle when no enhancement context is present. Set `STAGE_ENABLE_TOOL_LOOP=false` to force the loop off for an environment.
- **A successful first-cycle (toolless) task writes exactly 3 audit rows**: `guardrail_check_input`, `llm_call`, `guardrail_check_output` (LOAD writes none; TOOL_LOOP self-skips and writes none for an agent with empty `allowed_tools`). An agent WITH `allowed_tools` additionally runs the LLM↔MCP loop and writes `tool_call` / `tool_loop_limit` rows. WP12 adds `rag_query` / `tool_call` / `tool_loop_limit` / `context_truncated` step types (migration 0005 extends the CHECK enum — keep it in sync with `models/task.py STEP_TYPES`).
