...
def _workflow_dict(wf: repo.WorkflowRow) -> dict[str, Any]:
    return {
        ...
        "goal": wf.goal, "status": wf.status, "mode": wf.mode, "decomposition": wf.decomposition,
...
def _node_dict(n: repo.WorkflowTaskRow) -> dict[str, Any]:
    return {
...
async def submit_orchestration(
    ...
    workflow = await coord.create_run(
        principal, goal=body.goal, mode=body.mode,
...
async def get_orchestration_graph(
    ...
    return {"workflow": _workflow_dict(wf), "nodes": [_node_dict(n) for n in nodes]}
...
async def _run_sse(pool: Any, tenant_id: str, workflow_id: str) -> AsyncIterator[bytes]:
    """Poll the run + nodes; emit a frame on change; close on terminal (with a hard max duration)."""
    ...
    while loop.time() < deadline:
        ...
        snapshot = {"workflow": _workflow_dict(wf), "nodes": [_node_dict(n) for n in nodes]}
