...
The runtime role (``xagent_user``) is NOT a superuser and does NOT bypass RLS, so every
...
transaction-local — equivalent to ``SET LOCAL``). The RLS policies on
``xagent.agents`` / ``xagent.tasks`` / ``xagent.task_steps`` then admit only rows for
that tenant. ``xagent.outbox`` has NO RLS (internal cross-tenant publish queue).
"""
