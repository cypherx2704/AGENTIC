// Atlas project config for xAgent agent-runtime migrations (Phase 9A).
...
//   20260608_0002__seed.sql  — no-op (agents are created per-tenant at runtime)
//
...
variable "dev_url" {
  ...
  default = "docker://postgres/16/dev?search_path=xagent"
}

env "local" {
  url = var.url != "" ? var.url : "postgres://xagent_user:localdev@localhost:5432/cypherx_platform?search_path=xagent&sslmode=disable"
  dev = var.dev_url
  src = "file://schema.sql"
  migration {
    dir = "file://."
